...
public class SignManageDaoImp {
	private QueryRunner runner;

	public SignManageDaoImp() {
		runner = new QueryRunner(DataSourceManager.getSource());
	}

	public List<LogSignIn> queryById(String id) {
		String sql = String.format("select * from %s where %s=%s order by %s desc", LogSignIn.TABLE_NAME,
				LogSignIn.COLUMN_UID, id, LogSignIn.COLUMN_TIME);
		// TODO Auto-generated method stub
		List<LogSignIn> list = null;
		try {
			list = runner.query(sql, new BeanListHandler<LogSignIn>(LogSignIn.class));
	...
	public boolean updateInLog(String id, String place, String time, int count) {
		String sql = String.format("update %s set %s=%s, %s=%s,%s=%d where %s=%s", LogSignIn.TABLE_NAME,
